...
public class V8RttiService {
    ...
    public ProxyObject createNewObject(ClazzDefCompositeDO classDef, TaskContext taskContext, Boolean isPersist){
        ...
        if (isPersist) {
            ...
            objectAttrValueMap.put(FieldConstants.CLAZZ_GUID,classDef.getGuid());
            List<ClazzDefCompositeDO> parents = entityDepService.getClassDefWithParents(taskContext, classDef.getParentGuid());
            EntityStoreMessageDO messageDO = StoreUtil.getStoreMessageDO(classDef, parents, taskContext,objectAttrValueMap);
