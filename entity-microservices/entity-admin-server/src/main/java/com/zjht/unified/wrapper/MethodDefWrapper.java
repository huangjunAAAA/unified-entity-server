...
public class MethodDefWrapper {
    ...
    public static MethodDefWrapper build() {
        ...
        return new MethodDefWrapper();
    }

    public MethodDefVo entityVO(MethodDef methodDef) {
        if (methodDef == null)
            return null;
        MethodDefVo methodDefVo = new MethodDefVo();
        BeanUtils.copyProperties(methodDef, methodDefVo);
        ArrayList<MethodDefVo> collect = new ArrayList<MethodDefVo>();
        collect.add(methodDefVo);
        
		translateDict(collect);
        return methodDefVo;
    }

    public List<MethodDefVo> entityVOList(List<MethodDef> list) {
        List<MethodDefVo> collect = list.stream().map(entity -> {
            MethodDefVo methodDefVo = new MethodDefVo();
            BeanUtils.copyProperties(entity, methodDefVo);
            return methodDefVo;
	...
	public List<MethodDefVo> translateDict(List<MethodDefVo> list){
    ...
    public IPage<MethodDefVo> pageVO(IPage<MethodDef> source) {
        Page<MethodDefVo> page = new Page<MethodDefVo>();
  	...
  	public void validateMethodDef(MethodDef methodDef){
      
    }

    public void initMethodDef(MethodDef methodDef){
